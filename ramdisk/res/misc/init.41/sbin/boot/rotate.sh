...
if busybox [ $(busybox grep -c ro.sf.hwrotation=270 /system/build.prop) -eq 1 ]; then
  ...
  # busybox ln -sf /system/lib/hw/blackhawk/vendor-camera.espresso.so /system/lib/hw/camera.omap4.so
else
  ...
  # busybox ln -sf /system/lib/hw/blackhawk/vendor-camera.espresso10.so /system/lib/hw/camera.omap4.so
fi
